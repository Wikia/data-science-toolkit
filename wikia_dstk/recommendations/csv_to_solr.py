...
def get_args():
    ap = ArgumentParser()
    ap.add_argument('--solr-url', dest='solr_url', default='http://dev-search:8983/', help="The Solr endpoint")
...
def send_solr_update(args, docs):
    ...
    resp = requests.post(solr_url, data=json.dumps(docs))
    if resp.status != 200:
...
def main():
    ...
    for fl in files:
        ...
        for line in fl:
            split = line.split(',')
            batch.append({'id': split[0], args.recommendations_field: split[1:]})
